...
import { Sparkles, Save, ServerCrash, Database, Cpu, Settings, Activity, Plus, Loader2 } from 'lucide-react';
    ...
    return (
        <div className="p-8 pb-20 flex flex-col xl:flex-row gap-8">
            ...
            <div className="flex-[0.8] flex flex-col h-[calc(100vh-8rem)]">
                <div className="flex justify-between items-end mb-8 border-b border-rvs-steel/20 pb-4 shrink-0">
                    <div>
                ...
                <div className="flex flex-col gap-4 overflow-y-auto custom-scrollbar flex-1 pb-10 pr-2">
                    {agents.length === 0 && selectedAgentId !== 'new' && (
                        <div className="text-center p-8 bg-rvs-darker border border-rvs-steel/10 rounded-xl">
                            ...
                            <p className="text-xs text-rvs-steel/60">Create a new agent to connect to Ollama.</p>
                        </div>
                    )}
                        ...
                        return (
                            ...
                            >
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h3 className="text-xl font-black text-white tracking-widest uppercase mb-1">{agent.name}</h3>
                                        ...
                                        <div className="flex items-center gap-2 text-[10px] text-emerald-400 font-mono">
                                            <Cpu size={12} /> Model: {agent.llmModel}
                                        </div>
                                        {isSelected && (
                                            <div className="mt-4 flex items-center gap-3 pt-3 border-t border-rvs-steel/10">
                                                <div className="flex items-center gap-1 text-[10px] text-emerald-400 font-black uppercase tracking-widest">
                                                    <Activity size={12} /> Efficiency: {Math.floor(Math.random() * 20 + 80)}%
                                                </div>
                                                <div className="flex items-center gap-1 text-[10px] text-blue-400 font-black uppercase tracking-widest">
                                                    <Database size={12} /> Vectorized
                                                </div>
                                            </div>
                                        )}
                                    </div>
            ...
            <div className="flex-[1.2] rvs-card border border-rvs-steel/20 rounded-xl shadow-2xl flex flex-col h-[calc(100vh-8rem)] relative overflow-hidden">
                ...
                <div className="p-8 border-b border-rvs-steel/20 flex flex-col gap-2 shrink-0 relative bg-rvs-darker shadow-md z-10">
                    <div className="flex items-center justify-between">
                        ...
                        >
                            {isSaving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} {isSaving ? 'Saving...' : 'Save Config'}
                        </button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar pb-10">
                    ...
                    <div className="border border-emerald-500/20 bg-emerald-500/5 rounded-lg p-5">
                        <label className="text-xs font-black text-white uppercase tracking-widest mb-3 flex items-center gap-2">
                            <Cpu size={16} className="text-emerald-400" /> Language Model Assignment
                            {(models.length > 0) ? (
                                <span className="text-[9px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded font-black ml-2">Ollama Connected</span>
                            ) : (
                                <span className="text-[9px] bg-red-500/20 text-red-400 px-2 py-0.5 rounded font-black ml-2">Ollama Disconnected</span>
                            )}
                        </label>
                        <select
                            value={selectedModel}
                        ...
                        >
                            {models.length > 0 ? (
                                ...
                                <>
                                    <option value="qwen3.5:0.8b">Qwen 3.5 0.8B (Fallback)</option>
                                    <option value="llama3:8b">Llama 3 8B (Fallback)</option>
                                </>
                            )}
                        </select>
                        <p className="mt-3 text-xs text-emerald-400/80 italic font-mono flex items-center gap-2">
                            <Activity size={12} /> Local execution selected. Ensure hardware monitors are active.
                        </p>
                    </div>
                    ...
                    <div>
                        <label className="text-xs font-black text-rvs-steel uppercase tracking-widest mb-3 flex items-center justify-between">
                            ...
                            <span className="text-[10px] text-rvs-chrome opacity-80 bg-blue-500/20 text-blue-300 font-bold px-2 py-0.5 rounded border border-blue-500/30">Vector Index Ready</span>
                        </label>
                        <textarea
                            value={knowledgeBase}
                            onChange={(e) => setKnowledgeBase(e.target.value)}
                            className="w-full bg-black/20 border border-blue-500/30 rounded-lg p-4 text-sm text-blue-100 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 resize-none h-24 font-mono transition-colors shadow-[inset_0_0_20px_rgba(59,130,246,0.05)]"
                    ...
                    <div className="bg-rvs-darker/80 p-6 rounded-lg border border-purple-500/20 mt-8 relative overflow-hidden">
                        ...
                        <textarea
                            ...
                            placeholder="Input simulated lead text..."
                        />

                        <button
                            onClick={handleTest}
                            disabled={isLoading}
                            className="w-full bg-purple-600 text-white font-black tracking-widest uppercase py-3 rounded-lg hover:bg-purple-500 transition-colors disabled:opacity-50 flex justify-center items-center gap-3 shadow-[0_0_15px_rgba(168,85,247,0.3)] mt-2 border border-purple-400/50"
                        >
                            {isLoading ? (
                                <><Loader2 size={16} className="animate-spin" /> Querying Model...</>
                            ) : (
                                <><ServerCrash size={16} /> Execute Test Generation</>
                            )}
                        </button>
                    </div>
